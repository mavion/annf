...
entry main [iters] [patch_count] [n] [k]
    ...
    let (matches,_) = 
        loop (matches, matchl2) = (matches, matchl2) for i < iters do
            ...
            let candidates = find_candidates_all matches hash[i,:] hash[i,:] hash_table[i,:] hash_table[i,:] dimy dimy2 |> opaque
            -- find l2 distance of all candidates
            let candidatesl2 = map2 (\x ys -> map (\y -> dist2 wh[x,:] wh[y,:]) ys) (iota patch_count) candidates |> opaque
            ...
            let (matches', matchl2') = unzip (map4 (bruteForcePar) matches matchl2 candidates candidatesl2) |> opaque
            in (matches', matchl2')
    in matches