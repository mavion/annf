...
let cshANN [n] [m] [k] [j]
    ...
    let (matches,_) = 
        loop (matches, matchl2) = (matches, matchl2) for i < iters do
            ...
            let candidates = find_candidates_all matches hash_src[i,:] hash_trg[i,:] hash_table_src[i,:] hash_table_trg[i,:] dimy dimy2 |> opaque
            -- find l2 distance of all candidates
            let candidatesl2 = map2 (\x ys -> map (\y -> dist2 wh_src_trs[x,:] wh_trg_trs[y,:]) ys) (iota patch_count) candidates |> opaque
